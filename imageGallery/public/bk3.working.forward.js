...
function createImg(image, index) {
  ...
  tempImage.src = image;
  tempImage.setAttribute('draggable', false);
  // tempImage.setAttribute('ondragstart', 'drag(event)');
  // tempImage.setAttribute('ondragover', 'showDrop(event)');
...
function inserAfter(newNode, referenceNode) {
...
function drop(event) {
  ...
  inserAfter(document.getElementById(data), parentElement);
  ...
  */
  ...
  console.log('id.parentElement: ', id.parentElement);

  

  // clone an element
  let newelement = target.parentElement.cloneNode(true);

  // id.parentElement.appendChild(newelement, id);
  inserAfter(newelement, id.parentElement);
